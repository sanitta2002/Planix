...
export const getIssuesByProject = async (projectId: string) => {
    const response = await AxiosInstance.get(API_ROUTES.ISSUE.GET_ISSUES_BY_PROJECT.replace(":projectId", projectId));
    return response.data;
};

export const updateIssue = async (id: string, data: UpdateIssueProps) => {
    const response = await AxiosInstance.patch(API_ROUTES.ISSUE.UPDATE_ISSUE.replace(":issueId", id), data);
    return response.data;
};
